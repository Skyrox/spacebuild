MsgC(Color(255,255,255),"-----------------------[SERVER]----------------------\n")
MsgC(Color(255,170,100),"     CAF - Loading Resource Distribution Adddon      \n")
...
local function RequestResourceData(_, ply)
				...
				if num ~= -1 then
					...
					if resnames then
						for _, k in pairs(resnames) do
							local value, maxvalue = RD.GetResourceData(tmpdata.ent, k)
							data.resources[k] = {
								value = value,
								maxvalue = maxvalue
							}
	...
	elseif typ == REQUEST_NET then
		...
		if not data then
			...
			for k, v in pairs(tmpdata.resources) do
				local value, maxvalue = RD.GetNetResourceData(id, k)
				local localvalue, localmaxvalue = RD.GetNetResourceData(id, k, false)
				data.resources[k] = {
					value = value,
					maxvalue = maxvalue,
...
function RD.GetResourceData(ent, resource, sumconnectednets, ignorenet)
	...
	if ignorenet or index.network == 0 then
		if index.resources[resource] then
			return index.resources[resource].value, index.resources[resource].maxvalue
		end
		return 0, 0, 0
	end
	sumconnectednets = sumconnectednets or (sumconnectednets == nil)
